...
class ShoppingCart extends ItemList implements Visitor {
        ...
        while (it.hasNext()) {
            ...
            while (it2.hasNext()) {
                if (item.equals(it2.next())) {
                    ...
                    this.setBudget(this.getBudget() + 0.1*item.getprice());
    ...
    public void visit(MusicDepartment musicDepartment) {
        ...
        this.setBudget(this.getBudget() + 0.1*totalPrice);
    }
    ...
    public void visit(SoftwareDepartment softwareDepartment) {
        ...
        int ok = 0;
        while (it.hasNext()) {
            if (totalPrice + it.next().getprice() <= this.getBudget()) {
                ok = 1;
            }
        }
        if (ok != 1) {
            ...
            while (it2.hasNext()) {
               Item item = it2.next();
                ...
                if (ret == true) {
                    ...
                    this.setBudget(this.getBudget() + 0.2*item.getprice());
                    item.setprice(newPrice);
                }
            }
        }
        ...
        if(bigest_price < totalPrice){
            ...
            while (it2.hasNext()) {
                ...
                if (ret == true) {
                    ...
                    item.setprice(price);
                    this.setBudget(this.getBudget() + 0.15*item.getprice());
                }
            }
        }
        Double newBudget = this.getBudget() + totalPrice*0.05;
        this.setBudget(newBudget);
    }

    public boolean add(Item element) {
        if (this.getBudget() >= element.getprice() ) {
