...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	angle = angle + 5 * deltaTimeSeconds;
	polX = polX + 5 * deltaTimeSeconds;
	polY = abs(sin(polX));
	if (angle > 3.1415) {
		angle = angle - 3.1415;
	}
	...
	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(3.0f, 0.5f, -1.5f);
	...
	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(4.5, 0.5f, -1.5f);
	modelMatrix *= Transform3D::Translate(polX + stepX, polY, 0);
	RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
...
void Laborator4::OnInputUpdate(float deltaTime, int mods)
{
	if (window->KeyHold(GLFW_KEY_S)) {
		translateZ = translateZ + 10 * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_D)) {
		translateX = translateX + 10 * deltaTime;
	}
	...
	if (window->KeyHold(GLFW_KEY_R)) {
		translateY = translateY + 10 * deltaTime;
	}
	...
	if (window->KeyHold(GLFW_KEY_1)) {
		scaleX = scaleX - 2 * deltaTime;
		scaleY = scaleY - 2 * deltaTime;
		scaleZ = scaleZ - 2 * deltaTime;
	}
