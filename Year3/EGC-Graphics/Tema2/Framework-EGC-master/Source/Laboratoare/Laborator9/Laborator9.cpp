...
void Laborator9::Init()
{
	...
	// Create a simple quad
	{
		...
		{
			glm::vec2(0,   0),	// Top Right
			glm::vec2(0,  1),	// Bottom Right
			glm::vec2(1, 1),	// Bottom Left
			glm::vec2(1,  0),	// Top Left
		};
...
Texture2D* Laborator9::CreateRandomTexture(unsigned int width, unsigned int height)
{
	...
	for (int i = 0; i < width; i++) {
		for (int j = 0; j < height; j++) {
			if (isWhite) {
				data[(width * j + i) * 3 + 0] = 255;
				data[(width * j + i) * 3 + 1] = 255;
				data[(width * j + i) * 3 + 2] = 255;
			}
			else {
				data[(width * j + i) * 3 + 0] = 0;
				data[(width * j + i) * 3 + 1] = 0;
				data[(width * j + i) * 3 + 2] = 0;
			}
			isWhite = !isWhite;
		}
	}
