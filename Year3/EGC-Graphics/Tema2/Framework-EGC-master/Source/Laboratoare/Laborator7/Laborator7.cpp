...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint x_light_position = glGetUniformLocation(shader->program, "light_position");
	glUniform3fv(x_light_position, 1, glm::value_ptr(lightPosition));
	// TODO: Set eye position (camera position) uniform
	glm::vec3 eyePosition = GetSceneCamera()->transform->GetWorldPosition();

	GLint x_eye_position = glGetUniformLocation(shader->program, "eye_position");
	glUniform3fv(x_eye_position, 1, glm::value_ptr(eyePosition));
	// TODO: Set material property uniforms (shininess, kd, ks, object color) 
	GLint x_loc_materialkd = glGetUniformLocation(shader-> program, "material_kd");
	glUniform1f(x_loc_materialkd, materialKd);

	GLint x_loc_materialks = glGetUniformLocation(shader->program, "material_ks");
	glUniform1f(x_loc_materialks, materialKs);

	GLint x_loc_shiness = glGetUniformLocation(shader->program, "material_shininess");
	glUniform1i(x_loc_shiness, materialShininess);

	GLint x_loc_objectcolor = glGetUniformLocation(shader->program, "object_color");
	glUniform3fv(x_loc_objectcolor, 1, glm::value_ptr(color));
