...
#include <include/math.h>
	...
	public:
		...
		{
			position = position +glm::vec3(0, 0, distance);
		...
		void RotateFirstPerson_OX(float angle)
		{
			...
			// Use glm::rotate()
			glm::mat4 rotationOX = glm::rotate(glm::mat4(1.0f), angle, right);
			glm::vec4 newVecForw = glm::vec4(forward, 1.0);
			newVecForw = rotationOX * newVecForw;
			forward = glm::normalize(glm::vec3(newVecForw));
		...
		void RotateFirstPerson_OY(float angle)
		{
			...
			// Use glm::rotate()
			glm::mat4 rotationOY = glm::rotate(glm::mat4(1.0f), angle, glm::vec3(0, 1, 0));
			glm::vec4 newVecForw = glm::vec4(forward, 1.0);
			glm::vec4 newRight = glm::vec4(right, 1.0);
			newVecForw = rotationOY * newVecForw;
			newRight = rotationOY * newRight;

			forward = glm::normalize(newVecForw);
		...
		void RotateFirstPerson_OZ(float angle)
		{
			...
			// Don't forget to normalize the vectors
			glm::mat4 rotationOZ = glm::rotate(glm::mat4(1.0f), angle, forward);
			glm::vec4 newRight = glm::vec4(right, 1.0);

			newRight = rotationOZ * newRight;
			right = glm::normalize(glm::vec3(newRight));
			up = glm::cross(right, forward);
		}
