...
void Laborator5::Update(float deltaTimeSeconds)
{
	{
		if (!perspectiva) {
			projectionMatrix = glm::ortho(-2.0, 2.0, -5.0, 5.0, 0.0, 10.0);
		}
		else {
			projectionMatrix = glm::perspective(RADIANS(variabila), window->props.aspectRatio, 0.01f, 200.0f);
		}
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-6, 0.5, 0));
