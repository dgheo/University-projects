...
void Laborator3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
	if (window->KeyHold(GLFW_KEY_W)) {
		logicSpace.y = logicSpace.y + deltaTime * 3;
	}
	if (window->KeyHold(GLFW_KEY_S)) {
		logicSpace.y = logicSpace.y - deltaTime * 3;
	}
	if (window->KeyHold(GLFW_KEY_D)) {
		logicSpace.x = logicSpace.x + deltaTime * 3;
	}
	if (window->KeyHold(GLFW_KEY_A)) {
		logicSpace.x = logicSpace.x - deltaTime * 3;
	}
	if (window->KeyHold(GLFW_KEY_Z)) {
		logicSpace.height= logicSpace.height + deltaTime * 3;
		logicSpace.width = logicSpace.width + deltaTime * 3;
		logicSpace.x = logicSpace.x - deltaTime * 3 / 2;
		logicSpace.y = logicSpace.y - deltaTime * 3 / 2;
	}
